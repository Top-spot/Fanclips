...
import { User, Settings, LogOut, Edit3, Trophy, Film, Heart, Lock, Zap, ChevronRight } from "lucide-react";
...
export default function ProfilePage() {
  ...
  return (
    <div className="h-full overflow-y-scroll scrollbar-hide">
      ...
      <div className="mx-4 my-4 gradient-card border border-border rounded-2xl p-5 shadow-card">
        <div className="flex items-center gap-4 mb-4">
          <div className="relative">
            <Avatar className="w-16 h-16 border-2 border-electric/40">
          ...
          <button
            onClick={() => { setEditing(!editing); setUsername(profile.username); setTeam(profile.team ?? ""); setSection(profile.section ?? ""); setErrors({}); }}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { icon: <Heart className="w-4 h-4 text-destructive" />, value: totalLikes.toLocaleString(), label: "Likes" },
          ].map((stat) => (
      ...
      {/* Edit form */}
      {editing && (
        <div className="mx-4 mb-4 gradient-card border border-electric/20 rounded-2xl p-4 space-y-3 animate-slide-up">
