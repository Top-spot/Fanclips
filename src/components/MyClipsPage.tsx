...
export default function MyClipsPage() {
  ...
  const toggleHidden = async (clipId: string, nextHidden: boolean) => {
    const { error } = await supabase
      ...
      .eq("user_id", user!.id);
    if (!error) {
      setClips((prev) => prev.map((c) => (c.id === clipId ? { ...c, is_hidden: nextHidden } : c)));
    }
  };
    ...
    <div className="h-full overflow-y-scroll scrollbar-hide">
      ...
      {loading ? (
        ...
        <div className="px-4 space-y-3 pb-8">
          {clips.map((clip) => {
            const sc = clip.is_hidden ? statusConfig.hidden : statusConfig[clip.status] ?? statusConfig.processing;
            return (
                ...
                <div className="flex gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        onClick={() => void toggleHidden(clip.id, !clip.is_hidden)}
                        className="text-xs border border-border rounded-full px-2 py-0.5 text-muted-foreground hover:text-foreground"
                      >
