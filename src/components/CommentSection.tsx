...
export default function CommentSection({ clipId, clipOwnerId }: { clipId: string; clipOwnerId: string }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Create notification for clip owner via secure function
      if (clipOwnerId !== user.id) {
        supabase.rpc("notify_comment", {
          p_clip_id: clipId,
          p_commenter_username: profile?.username || "someone",
        }).then(({ error: notifErr }) => {
          if (notifErr) console.error("Comment notification failed:", notifErr);
        });
      }
    } catch (err) {
